...
def dense_to_sparse(adj):
    ...
    edge_index = torch.ones_like(flatten_adj).nonzero().t() #changed
    edge_attr = flatten_adj
    offset = torch.arange(
        start=0,
...
class GraphLearner(nn.Module):
    ...
    def forward(self, processed_input):
        ...
        b_edge_index = dense_to_sparse(b_adj)

        b_edge_weights = b_adj.flatten() #new line added to fix gpu bug
