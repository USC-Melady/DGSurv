...
class PyTorchDeep(Explainer):
    ...
    def __init__(self, model, data, mask):
        ...
        with torch.no_grad():
            ...
            hazard, representation, graph, assignment_mat = model(*data, mask)
    ...
    def gradient(self, idx, inputs, mask):
        import torch
        self.model.zero_grad()
        #X = [x.requires_grad_() for x in inputs]
        X = [inputs[list(inputs)[k]].requires_grad_() for k in range(len(inputs))]  #don't require gradients for the clinical categorical variables
        #outputs = self.model(inputs, mask)
        hazard, representation, graph, assignment_mat = self.model(inputs, mask)
        outputs = hazard['hazard']
        ...
        if self.interim:
            ...
            for idx, x in enumerate(X):
                grad = torch.autograd.grad(selected, x, retain_graph=True, allow_unused=True)[0]
                if grad is not None:
                    grad = grad.cpu().numpy()
                else:
                    grad = torch.zeros_like(X[idx]).cpu().numpy()
    ...
    def shap_values(self, X, mask, ranked_outputs=None, output_rank_order="max", check_additivity=False):
        import torch
        # X ~ self.model_input
        # X_data ~ self.data
        ...
        X_list = [X[list(X)[k]].detach().to(self.device) for k in range(len(X))]

        model_output_values = None
        if ranked_outputs is not None and self.multi_output:
            with torch.no_grad():
                model_output_values = self.model(*X, mask)
                #hazard, representation, graph, assignment_mat = model(*data, mask)
                #model_output_values = hazard['hazard']
            ...
            model_output_ranks = (torch.ones((X[list(X)[0]].shape[0], self.num_outputs)).int() *
        ...
        for i in range(model_output_ranks.shape[1]):
            ...
            if self.interim:
                ...
                for k in range(len(X)):
                    phis.append(np.zeros(X[list(X)[k]].shape))
            for j in range(X[list(X)[0]].shape[0]):
                # tile the inputs to line up with the background data samples
                tiled_X = [X_list[t][j:j + 1].repeat(
                                   (self.data[0][list(X)[t]].shape[0],) + tuple([1 for k in range(len(X_list[t].shape) - 1)])) for t
                           in range(len(X))]
                joint_x = [torch.cat((tiled_X[t], self.data[0][list(X)[t]]), dim=0) for t in range(len(X))]
                # run attribution computation graph
                feature_ind = model_output_ranks[j, i]
                joint_x_dict = X.copy()
                for m in range(len(X)):
                    key = list(X)[m]
                    joint_x_dict[key] = joint_x[m]
                mask_cat = torch.cat((self.mask, self.mask), dim=0)
                sample_phis = self.gradient(feature_ind, joint_x_dict, mask_cat)
                ...
                if self.interim:
                    ...
                    for t in range(len(X)):
                        phis[t][j] = (torch.from_numpy(sample_phis[t][self.data[0][list(X)[t]].shape[0]:]).to(self.device) * (X_list[t][j: j + 1] - self.data[0][list(X)[t]])).cpu().detach().numpy().mean(0)
        ...
        if check_additivity:
            if model_output_values is None:
                with torch.no_grad():
                    ...
                    hazard, representation, graph, assignment_mat = self.model(X, mask)
                    model_output_values = hazard['hazard']
